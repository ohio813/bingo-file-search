...
void* MemoryPool::alloc (size_t Size)
{
    ...
    {
        p = allocWithOutSync (Size);
    }
    return p;
}
void* MemoryPool::allocWithOutSync (size_t Size)
{
    ...
    if (ptrBlock != m_memBlocks.end())
    {
        ...
        else
        {
            ...
            else
            {
                ...
                void* pNewSrc = allocWithOutSync (Size);
...
void MemoryPool::DumpPoolBin (const std::wstring& fileName)
{
    HANDLE hFile = CreateFileW (fileName.c_str(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
void MemoryPool::DumpPoolTxt (const std::wstring& fileName)
{
