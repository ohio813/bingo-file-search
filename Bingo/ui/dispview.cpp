...
{
    setSql();
    this->model.setQuery (this->model.sql, db);
...
void DispView::setSql()
{
    ...
    if (!uidata_searchwidget->getSearchKeyWord().isEmpty())
    {
        if (!uidata_searchwidget->isFullPathMatchFilterOn())
        {
            ...
            pathFilter = nameFilter.arg().arg (uidata_searchwidget->getSearchKeyWord());
        }

        if (uidata_searchwidget->isFullPathMatchFilterOn())
        {
            pathFilter = "Path_%1.path like '\%%2\%' ";
            pathFilter = pathFilter.arg().arg (uidata_searchwidget->getSearchKeyWord());
        }
    }
    ...
    if (uidata_searchwidget->isDateCreatedFilterOn())
    {
        ...
        createTimeFilter = sizeFilter.arg ("%1", _begin_create_date, _end_create_date);
    }
    ...
    if (uidata_searchwidget->isDateModifiedFilterOn())
    {
        ...
        createTimeFilter = "Master_%1.writeTime>%2 and Master_%1.writeTime<%3";
        createTimeFilter = sizeFilter.arg ("%1", _begin_mod_time, _end_mod_time);
    }
    ...
    for (QList<char>::iterator _ptr = _drivers.begin();
            ...
            _ptr++)
    {
        this->model.sql = (this->sql) + _basic_sql.arg (*_ptr) + " union ";
    }

    this->model.sql = this->model.sql.mid (0, this->sql.length() - 6);
}
///:~
